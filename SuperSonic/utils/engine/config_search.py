...
class ConfigSearch:
    ...
    def __init__(self, task_config):
        ...
        self.num_samples = 1
        self.sched = ASHAScheduler(metric="episode_reward_mean", mode="max", max_t=1000)
        self.reporter = CLIReporter()

    def PPO(self, task_config, environment_path):
        self.lamda = 0.95
        self.kl_coeff = 0.2
        self.vf_clip_param = 30.0
        ...
        # get the best config
        best_config, best_metric_score = analysis.get_best_config(
    ...
    def QLearning(self, task_config, environment_path):
        ...
        analysis = tune.run(
            ...
            config={
                ...
                "learning_starts": tune.uniform(0.001, 1.0),
        ...
        for i in range(iterations):
            try:
                if policy_algorithm == "MCTS":
                    best_config, reward = ConfigSearch(task_config).MCTS(
                        task_config, environment_path
                    )
                if policy_algorithm == "PPO":
                    best_config, reward = ConfigSearch(task_config).PPO(
                        task_config, environment_path
                    )
                if policy_algorithm == "DQN":
                    best_config, reward = ConfigSearch(task_config).DQN(
                        task_config, environment_path
                    )
                if policy_algorithm == "QLearning":
                    best_config, reward = ConfigSearch(task_config).QLearning(
                        task_config, environment_path
                    )
                if policy_algorithm == "APPO":
                    best_config, reward = ConfigSearch(task_config).APPO(
                        task_config, environment_path
                    )
                if policy_algorithm == "A2C":
                    best_config, reward = ConfigSearch(task_config).A2C(
                        task_config, environment_path
                    )
                if policy_algorithm == "A3C":
                    best_config, reward = ConfigSearch(task_config).A3C(
                        task_config, environment_path
                    )
                if policy_algorithm == "ARS":
                    best_config, reward = ConfigSearch(task_config).ARS(
                        task_config, environment_path
                    )
                if policy_algorithm == "ES":
                    best_config, reward = ConfigSearch(task_config).ES(
                        task_config, environment_path
                    )
                if policy_algorithm == "MARWIL":
                    best_config, reward = ConfigSearch(task_config).MARWIL(
                        task_config, environment_path
                    )
                if policy_algorithm == "PG":
                    best_config, reward = ConfigSearch(task_config).PG(
                        task_config, environment_path
                    )
                if policy_algorithm == "SimpleQ":
                    best_config, reward = ConfigSearch(task_config).SimpleQ(
                        task_config, environment_path
                    )
            except:
                pass
