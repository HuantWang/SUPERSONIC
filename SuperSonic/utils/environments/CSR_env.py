...
class csr_rl:
    ...
    def __init__(self, env_config):
        ...
        self.statefun = env_config.get("state_function")
        self.rewardfun = env_config.get("reward_function")
        if self.rewardfun != "runtime":
            self.env_reg = "llvm-autophase-" + self.rewardfun + "-v0"
        else:
            self.env_reg = "llvm-autophase-ic-v0"
        self.env = gym.make(
            self.env_reg,  # selects the compiler to use
            # make_benchmark(
            #     [self.benchmarks]
            # ),  # selects the program to compile
        )
        self.env.reset(self.benchmarks)
        if self.rewardfun == "runtime":
            self.env.runtime_observation_count = 10
            self.base_runtime = np.mean(self.env.observation.Runtime())
    ...
    def reset(self):
        ...
        self.episode_reward = 0
        return {
            "obs": self.env.reset(self.benchmarks),
    ...
    def step(self, action):
        ...
        with open(f"{self.pass_path}/{benchmarks}_pass.txt", "a+") as f:
            ...
            f.write(
                f"time={used_time:.3},action_len={len(self.env.actions)},episode_reward={self.episode_reward:.3%},reward_all={self.view:.3%}\r\n"
            )
        try:
        ...
        obs = observation_function(self.env, self.statefun).get_observation()

        if self.rewardfun == "runtime":
            try:
                self.env.runtime_observation_count = 10
                rew = self.base_runtime / np.mean(self.env.observation.Runtime())
                ...
                print(
                    "Benchmark ",
                    self.benchmarks,
                    " cannot be compiled, please remove this benchmark from your dataset",
                )

            # print("benchmark", self.env.benchmark, "a ", self.benchmarks)
            # print("IsRunnable",self.env.observation.IsRunnable())
            # print("reward", np.mean(self.env.observation.Runtime()))

        else:
            self.running_reward += rew
            self.episode_reward = self.episode_reward + rew

        if self.episode_reward > self.view:
            self.view = self.episode_reward
        ...
        if len(self.env.actions) > self.env.action_space.n:
            ...
            if used_time > self.deadline and not self.flag:
                self.flag = True
                with open(f"{self.log_path}/{benchmarks}_result.txt", "a+") as f:
        ...
        self.score = self.running_reward
    ...
    def get_state(self):
        ...
        """
        env = self.env.fork()
        return env, self.running_reward
