...
class CNNLayerVisualization:
    ...
    def visualise_layer_with_hooks(self):
        ...
        processed_image = (
            processed_image.unsqueeze(0).unsqueeze(0).detach().requires_grad_(True)
        )
...
def filter_vis(flags):
    ...
    if len(paths) > 1:
        logging.warning(
            "More than one model specified for filter visualisation. "
...
def parallel_filter_calc(combined_input):
    ...
    if type(comparison) is str:
        return single_filter_comp(
            models[comparison], models[model_name], not flags.comp_no_optimal
        )
    return single_filter_comp(
        comparison[model_name], models[model_name], not flags.comp_no_optimal
    )
...
def filter_comp(flags):
    ...
    if len(base_path) > 1:
        logging.warning(
            "More than one path specified for filter progress visualization. "
    ...
    for model_name, path in intermediate_paths:
        ...
        if flags.match_num_models and "MultiTask" in model_name:
            index = list(
                np.round(np.linspace(0, 50 - 1, flags.comp_num_models)).astype(int)
            )
            index.append(len(checkpoints) - 1)
        else:
            index = np.round(
                np.linspace(0, len(checkpoints) - 1, flags.comp_num_models)
            ).astype(int)
        selected_models = [
            os.path.join(base_path, model_name, "intermediate", checkpoints[i][1])
            for i in index
        ]
    ...
    single_multi_data = {
        n: {s: {t: [] for t in ["sum", "mean"]} for s in ["default", "optimal"]}
        for n in single_task_names
    }
    single_single_data = {
        n: {s: {t: [] for t in ["sum", "mean"]} for s in ["default", "optimal"]}
        for n in single_task_names
        if n != flags.comp_single_single_model
    }
    ...
    multi_multipop_data = {
        s: {t: [] for t in ["sum", "mean"]} for s in ["default", "optimal"]
    }
    time_steps_data = {
        n: {s: {t: [] for t in ["sum", "mean"]} for s in ["default", "optimal"]}
    ...
    for t in range(flags.comp_num_models + (1 if flags.match_num_models else 0)):
        ...
        logging.info(
            "Loading checkpoints for all models ({}/{}).".format(
                t + 1, flags.comp_num_models
            )
        )
        models = {}
            ...
            models[model_name] = load_models(
                [
                    model_paths[
                        ...
                        - (
                            1
                            if t == flags.comp_num_models
                            and "multi" not in model_name.lower()
                            else 0
                        )
                    ]
                ]
        ...
        if "single_multi" in flags.comp_between:
            logging.info(
                "Comparing single-task and vanilla multi-task models ({}/{}).".format(
                    t + 1, flags.comp_num_models
                )
            )
            with mp.Pool(min(len(single_task_names), os.cpu_count())) as pool:
                full_data = pool.map(
                    parallel_filter_calc,
                    [(models, mn, multi_task_name) for mn in single_task_names],
                )
            for model_name, data in zip(single_task_names, full_data):
                dist, dd_data, od_data = data
                single_multi_data[model_name]["default"]["sum"].append(
                    np.stack([d[1] for d in dd_data])
                )
                single_multi_data[model_name]["default"]["mean"].append(
                    np.stack([d[2] for d in dd_data])
                )
                single_multi_data[model_name]["optimal"]["sum"].append(
                    np.stack([d[1] for d in od_data])
                )
                single_multi_data[model_name]["optimal"]["mean"].append(
                    np.stack([d[2] for d in od_data])
                )

        # compare one single-task model with all the others
        if "single_single" in flags.comp_between:
            logging.info(
                "Comparing {} single-task with all other single-task models ({}/{}).".format(
                    flags.comp_single_single_model, t + 1, flags.comp_num_models
                )
            )
            with mp.Pool(min(len(single_task_names) - 1, os.cpu_count())) as pool:
                full_data = pool.map(
                    parallel_filter_calc,
                    [
                        (models, mn, flags.comp_single_single_model)
                        for mn in single_task_names
                        if mn != flags.comp_single_single_model
                    ],
                )
            for model_name, data in zip(
                [n for n in single_task_names if n != flags.comp_single_single_model],
                full_data,
            ):
                dist, dd_data, od_data = data
                single_single_data[model_name]["default"]["sum"].append(
                    np.stack([d[1] for d in dd_data])
                )
                single_single_data[model_name]["default"]["mean"].append(
                    np.stack([d[2] for d in dd_data])
                )
                single_single_data[model_name]["optimal"]["sum"].append(
                    np.stack([d[1] for d in od_data])
                )
                single_single_data[model_name]["optimal"]["mean"].append(
                    np.stack([d[2] for d in od_data])
                )
        ...
        if "single_multipop" in flags.comp_between:
            logging.info(
                "Comparing single-task and multi-task PopArt models ({}/{}).".format(
                    t + 1, flags.comp_num_models
                )
            )
            with mp.Pool(min(len(single_task_names), os.cpu_count())) as pool:
                full_data = pool.map(
                    parallel_filter_calc,
                    [(models, mn, multi_task_popart_name) for mn in single_task_names],
                )
            for model_name, data in zip(single_task_names, full_data):
                dist, dd_data, od_data = data
                single_multipop_data[model_name]["default"]["sum"].append(
                    np.stack([d[1] for d in dd_data])
                )
                single_multipop_data[model_name]["default"]["mean"].append(
                    np.stack([d[2] for d in dd_data])
                )
                single_multipop_data[model_name]["optimal"]["sum"].append(
                    np.stack([d[1] for d in od_data])
                )
                single_multipop_data[model_name]["optimal"]["mean"].append(
                    np.stack([d[2] for d in od_data])
                )
        ...
        if "multi_multipop" in flags.comp_between:
            logging.info(
                "Comparing vanilla multi-task and multi-task PopArt models ({}/{}).".format(
                    t + 1, flags.comp_num_models
                )
            )
            dist, dd_data, od_data = single_filter_comp(
                models[multi_task_popart_name],
                models[multi_task_name],
                not flags.comp_no_optimal,
            )
            multi_multipop_data["default"]["sum"].append(
                np.stack([d[1] for d in dd_data])
            )
            multi_multipop_data["default"]["mean"].append(
                np.stack([d[2] for d in dd_data])
            )
            multi_multipop_data["optimal"]["sum"].append(
                np.stack([d[1] for d in od_data])
            )
            multi_multipop_data["optimal"]["mean"].append(
                np.stack([d[2] for d in od_data])
            )

        if "time_steps" in flags.comp_between:
            logging.info(
                "Comparing between time steps ({}/{}).".format(
                    t + 1, flags.comp_num_models
                )
            )
            if time_steps_last_models:
                with mp.Pool(min(len(model_names), os.cpu_count())) as pool:
                    full_data = pool.map(
                        parallel_filter_calc,
                        [(models, mn, time_steps_last_models) for mn in model_names],
                    )
                for model_name, data in zip(model_names, full_data):
                    dist, dd_data, od_data = data
                    time_steps_data[model_name]["default"]["sum"].append(
                        np.stack([d[1] for d in dd_data])
                    )
                    time_steps_data[model_name]["default"]["mean"].append(
                        np.stack([d[2] for d in dd_data])
                    )
                    time_steps_data[model_name]["optimal"]["sum"].append(
                        np.stack([d[1] for d in od_data])
                    )
                    time_steps_data[model_name]["optimal"]["mean"].append(
                        np.stack([d[2] for d in od_data])
                    )
    ...
    def update_dict(d):
        if type(d) == list:
            if type(d[0]) == list:
                result = [
                    np.stack([d[time][filter] for time in range(len(d))])
                    for filter in range(len(d[0]))
                ]
            else:
                result = np.stack(d)
            return result
    ...
    for file_name, dictionary in all_data.items():
        ...
        full_path = os.path.join(
            ...
            "filter_comp",
            "{}_{}".format(
                flags.comp_num_models, "match" if flags.match_num_models else "no_match"
            ),
            "{}{}.pkl".format(
                file_name,
                ""
                if file_name != "single_single"
                else "_{}".format(flags.comp_single_single_model),
            ),
        )
...
def filter_comp_plot(flags):
    ...
    for f in os.listdir(flags.load_path):
        if f.endswith(".pkl") and (
            f[:-4] in comparison_choices or "single_single" in f
        ):
    ...
    for comp_choice, current_data in all_data.items():
        if comp_choice == "multi_multipop":
            ...
            plt.ylabel(
                "SSD ({}) between corresponding filters ({} match)".format(
                    flags.plot_metric_type, flags.plot_match_type
                )
            )
            ...
            fig, ax = plt.subplots(
                nrows=single_task_rows,
                ncols=single_task_cols,
            ...
            for i in range(len(ax)):
                for j in range(len(ax[i])):
                    ...
                    data_default = current_data[
                        list(current_data.keys())[i * single_task_cols + j]
                    ]["default"]["mean"]
                    data_optimal = current_data[
                        list(current_data.keys())[i * single_task_cols + j]
                    ]["optimal"]["mean"]
                    ratio = np.divide(data_default, data_optimal)
                    """
            ...
            fig.suptitle(
                plot_titles[comp_choice]
                if comp_choice != "single_single"
                else plot_titles[comp_choice].format(
            ...
            if flags.save_figures:
                plt.savefig(
                    os.path.join(
                        ...
                        "{}_{}_{}{}.png".format(
                            comp_choice,
                            flags.plot_match_type,
                            flags.plot_metric_type,
                            ""
                            if comp_choice != "single_single"
                            else list(
                                mn for mn in single_task_names if mn not in current_data
                            )[0],
                        ),
                    )
                )
        ...
        for f_idx, f in enumerate(filters):
            ...
            for oc_idx, out_channel in enumerate(f):
                for ic_idx, in_channel in enumerate(out_channel):
                    s = (
                        (in_channel - filter_list[0][f_idx][oc_idx][ic_idx]) ** 2
                    ).sum()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default="~/logs/resnet_vis", help=".")
    parser.add_argument(
        "--mode",
        type=str,
        default="filter_vis",
    ...
    parser.add_argument(
        "--comp_single_single_model",
        type=str,
        default="Carnival",
        choices=single_task_names,
