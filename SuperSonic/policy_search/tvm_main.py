...
def getParser():
    ...
    parser.add_argument("--num_buffers", type=int, default="32",
                        help="num_buffers")
    parser.add_argument("--env", type=str, default="BanditHalideEnv-v0",
...
def main(flags):
    logs_path = f'logs/{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}'
    assert os.system(f"mkdir -p {logs_path}") == 0, "create {logs_path} error!!! return 0"
    ...
    flags.Dataset = policy_define.SuperOptimizer(
        datapath="tasks/tvm/test"
        # datapath="tasks/stoke/example/p04"
    ).cross_valid()
