...
configs_from_yaml = yaml.load(
    open(os.path.join(os.path.dirname(sys.argv[0]), "../../../build.yaml"))
)["configs"].keys()
...
def threads_required(scenario_json, where, is_tsan):
    ...
    if (
        scenario_json["%s_config" % where]["%s_type" % where]
        == "ASYNC_%s" % where.upper()
    ):
        return scenario_json["%s_config" % where].get("async_%s_threads" % where, 0)
    return (
        scenario_json["client_config"]["outstanding_rpcs_per_channel"]
        * scenario_json["client_config"]["client_channels"]
    )
...
def guess_cpu(scenario_json, is_tsan):
    ...
    if client == 0 or server == 0:
        return "capacity"
    return scenario_json["num_clients"] * client + scenario_json["num_servers"] * server
...
print yaml.dump(
    {
        ...
        + [
            {
                "name": "json_run_localhost",
                "shortname": "json_run_localhost:%s_low_thread_count"
                % scenario_json["name"],
                "args": [
                    "--scenarios_json",
                ...
                "exclude_configs": sorted(
                    c for c in configs_from_yaml if c not in ("tsan", "asan")
                ),
