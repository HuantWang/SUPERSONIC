...
uds_fixture_options = default_unsecure_fixture_options._replace(
    dns_resolver=False, platforms=["linux", "mac", "posix"], exclude_iomgrs=["uv"]
)
...
END2END_TESTS = {
    ...
    "compressed_payload": default_test_options._replace(
        proxyable=False, needs_compression=True
    ),
    "connectivity": connectivity_test_options._replace(
        needs_names=True, proxyable=False, cpu_cost=LOWCPU, exclude_iomgrs=["uv"]
    ),
    ...
    "disappearing_server": connectivity_test_options._replace(
        flaky=True, needs_names=True
    ),
    ...
    "hpack_size": default_test_options._replace(
        proxyable=False, traceable=False, cpu_cost=LOWCPU
    ),
    "high_initial_seqno": default_test_options._replace(cpu_cost=LOWCPU),
    "idempotent_request": default_test_options,
    "invoke_large_request": default_test_options,
    "keepalive_timeout": default_test_options._replace(
        proxyable=False, cpu_cost=LOWCPU, needs_http2=True
    ),
    "large_metadata": default_test_options,
    "max_concurrent_streams": default_test_options._replace(
        proxyable=False, cpu_cost=LOWCPU, exclude_inproc=True
    ),
    "max_connection_age": default_test_options._replace(
        cpu_cost=LOWCPU, exclude_inproc=True
    ),
    ...
    "retry": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_cancellation": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_disabled": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_exceeds_buffer_size_in_initial_batch": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_exceeds_buffer_size_in_subsequent_batch": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_non_retriable_status": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_non_retriable_status_before_recv_trailing_metadata_started": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    ...
    "retry_streaming": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_streaming_after_commit": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_streaming_succeeds_before_replay_finished": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
    "retry_throttled": default_test_options._replace(
        cpu_cost=LOWCPU, needs_client_channel=True, proxyable=False
    ),
...
def main():
    sec_deps = ["grpc_test_util", "grpc", "gpr_test_util", "gpr"]
    unsec_deps = ["grpc_test_util_unsecure", "grpc_unsecure", "gpr_test_util", "gpr"]
    json = {
