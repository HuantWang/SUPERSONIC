...
def add_nanopb_builders(env):
    ...
    def run_test(target, source, env):
        ...
        pipe = subprocess.Popen(
            args, stdin=infile, stdout=open(str(target[0]), "w"), stderr=sys.stderr
        )
        result = pipe.wait()
        if result == 0:
            print "\033[32m[ OK ]\033[0m   Ran " + args[0]
        else:
            print "\033[31m[FAIL]\033[0m   Program " + args[0] + " returned " + str(
                result
            )
        return result
    ...
    def compare_files(target, source, env):
        ...
        if data1 == data2:
            print "\033[32m[ OK ]\033[0m   Files equal: " + str(
                source[0]
            ) + " and " + str(source[1])
            return 0
        else:
            print "\033[31m[FAIL]\033[0m   Files differ: " + str(
                source[0]
            ) + " and " + str(source[1])
            return 1
        ...
        for pattern in patterns:
            if pattern.strip():
                ...
                if not status and not invert:
                    print "\033[31m[FAIL]\033[0m   Pattern not found in " + str(
                        source[0]
                    ) + ": " + pattern
                    return 1
                elif status and invert:
                    print "\033[31m[FAIL]\033[0m   Pattern should not exist, but does in " + str(
                        source[0]
                    ) + ": " + pattern
                    return 1
        else:
