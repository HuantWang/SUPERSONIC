...
class EncodedSize:
    ...
    def __mul__(self, other):
        if isinstance(other, int):
            return EncodedSize(
                self.value * other, [str(other) + "*" + s for s in self.symbols]
            )
        else:
...
class Field:
    ...
    def default_decl(self, declaration_only=False):
        ...
        if declaration_only:
            return "extern const %s %s_default%s;" % (
                ctype,
                self.struct_name + self.name,
                array_decl,
            )
        else:
            return "const %s %s_default%s = %s;" % (
                ctype,
                self.struct_name + self.name,
                array_decl,
                default,
        ...
        if self.pbtype == "MESSAGE":
            if self.rules == "REPEATED" and self.allocation == "STATIC":
                ...
                if self.anonymous:
                    ...
                    check.append(
                        "pb_membersize(%s, %s.%s)"
                        % (self.struct_name, self.union_name, self.name)
                    )
            else:
...
class Message:
    def __init__(self, names, desc, message_options):
        ...
        for f in desc.field:
            ...
            if (
                hasattr(f, "oneof_index")
                and f.HasField("oneof_index")
                and f.oneof_index not in no_unions
            ):
    ...
    def parse(self):
        ...
        for enum in self.fdesc.enum_type:
            enum_options = get_nanopb_suboptions(
                enum, self.file_options, base_name + enum.name
            )
