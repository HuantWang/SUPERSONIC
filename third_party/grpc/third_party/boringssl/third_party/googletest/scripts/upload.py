...
class AbstractRpcServer(object):
    ...
    """
        ...
        req = self._CreateRequest(
            "http://%s/_ah/login?%s" % (self.host, urllib.urlencode(args))
        )
        try:
            response = self.opener.open(req)
        except urllib2.HTTPError, e:
...
group.add_option(
    ...
    metavar="MESSAGE",
    default=None,
    help="A message to identify the patch. " "Will prompt if omitted.",
)
group.add_option(
    "-i",
    "--issue",
...
class SubversionVCS(VersionControlSystem):
    ...
    def GetBaseFile(self, filename):
        ...
        if status[0] == "A" and status[3] != "+":
            ...
            mimetype = RunShell(
                ["svn", "propget", "svn:mime-type", filename], silent_ok=True
            )
            base_content = ""
...
class MercurialVCS(VersionControlSystem):
    ...
    def GetUnknownFiles(self):
        ...
        args = []
        status = RunShell(
            ["hg", "status", "--rev", self.base_rev, "-u", "."], silent_ok=True
        )
        unknown_files = []
    ...
    def GetBaseFile(self, filename):
        ...
        if status != "A":
            base_content = RunShell(
                ["hg", "cat", "-r", self.base_rev, oldrelpath], silent_ok=True
            )
...
def GuessVCS(options):
  ...
  """
    ...
    try:
        out, returncode = RunShellWithReturnCode(
