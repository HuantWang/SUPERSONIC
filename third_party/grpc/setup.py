...
# in Cython modules.
ENABLE_CYTHON_TRACING = os.environ.get("GRPC_PYTHON_ENABLE_CYTHON_TRACING", False)
...
if EXTRA_ENV_COMPILE_ARGS is None:
    ...
    if "win32" in sys.platform and sys.version_info < (3, 5):
        ...
        EXTRA_ENV_COMPILE_ARGS += (
            " -std=gnu99 -fvisibility=hidden -fno-wrapv -fno-exceptions"
        )
...
if "darwin" in sys.platform and PY3:
    ...
    if mac_target and (
        pkg_resources.parse_version(mac_target) < pkg_resources.parse_version("10.7.0")
    ):
        os.environ["MACOSX_DEPLOYMENT_TARGET"] = "10.7"
        os.environ["_PYTHON_HOST_PLATFORM"] = re.sub(
            r"macosx-[0-9]+\.[0-9]+-(.+)", r"macosx-10.7-\1", util.get_platform()
        )
...
def cython_extensions_and_necessity():
    ...
    return (
        commands.try_cythonize(
            extensions, linetracing=ENABLE_CYTHON_TRACING, mandatory=BUILD_WITH_CYTHON
        ),
        need_cython,
    )
...
SETUP_REQUIRES = (
    INSTALL_REQUIRES + ("sphinx>=1.3", "sphinx_rtd_theme>=0.1.8", "six>=1.10",)
