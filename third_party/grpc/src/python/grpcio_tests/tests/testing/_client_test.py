...
class ClientTest(unittest.TestCase):
    ...
    def test_successful_unary_unary(self):
        ...
        self.assertIs(
            application_return_value.kind, _client_application.Outcome.Kind.SATISFACTORY
        )
    ...
    def test_successful_unary_stream(self):
        ...
        invocation_metadata, request, rpc = self._fake_time_channel.take_unary_stream(
    ...
    def test_successful_stream_unary(self):
        ...
        rpc.requests_closed()
        rpc.terminate(
            _application_common.STREAM_UNARY_RESPONSE, (), grpc.StatusCode.OK, ""
        )
        application_return_value = application_future.result()

        self.assertEqual(_application_common.STREAM_UNARY_REQUEST, first_request)
        self.assertEqual(_application_common.STREAM_UNARY_REQUEST, second_request)
        self.assertEqual(_application_common.STREAM_UNARY_REQUEST, third_request)
        self.assertIs(
            application_return_value.kind, _client_application.Outcome.Kind.SATISFACTORY
        )
    ...
    def test_successful_stream_stream(self):
        ...
        self.assertEqual(_application_common.STREAM_STREAM_REQUEST, first_request)
        self.assertEqual(_application_common.STREAM_STREAM_REQUEST, second_request)
        self.assertIs(
            application_return_value.kind, _client_application.Outcome.Kind.SATISFACTORY
        )
    ...
    def test_concurrent_stream_stream(self):
        ...
        for requests_of_one_rpc in requests.values():
            for request in requests_of_one_rpc:
                self.assertEqual(_application_common.STREAM_STREAM_REQUEST, request)
        self.assertIs(
            application_return_value.kind, _client_application.Outcome.Kind.SATISFACTORY
        )
    ...
    def test_cancelled_unary_unary(self):
        ...
        invocation_metadata, request, rpc = self._fake_time_channel.take_unary_unary(
            _application_testing_common.FIRST_SERVICE_UNUN
        )
        rpc.send_initial_metadata(())
        rpc.cancelled()
    ...
    def test_status_stream_unary(self):
        ...
        for rpc in rpcs:
            ...
            rpc.take_request()
            rpc.requests_closed()
            rpc.send_initial_metadata((("my_metadata_key", "My Metadata Value!",),))
        for rpc in rpcs[:-1]:
            rpc.terminate(
                _application_common.STREAM_UNARY_RESPONSE, (), grpc.StatusCode.OK, ""
            )
    ...
    def test_status_stream_stream(self):
        ...
        self.assertEqual(_application_common.STREAM_STREAM_REQUEST, second_request)
        self.assertIs(
            application_return_value.kind, _client_application.Outcome.Kind.RPC_ERROR
        )
    ...
    def test_infinite_request_stream_real_time(self):
        ...
        second_request = rpc.take_request()
        third_request = rpc.take_request()
        self._real_time.sleep_for(_application_common.INFINITE_REQUEST_STREAM_TIMEOUT)
        rpc.terminate(
            _application_common.STREAM_UNARY_RESPONSE,
            (),
