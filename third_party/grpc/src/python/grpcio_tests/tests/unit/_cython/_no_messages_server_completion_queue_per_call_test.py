...
class Test(_common.RpcTest, unittest.TestCase):
    def _do_rpcs(self):
        ...
        client_events_future = test_utilities.SimpleFuture(
            lambda: [self.channel.next_call_event(), self.channel.next_call_event(),]
        )
        ...
        with server_call_condition:
            server_send_initial_metadata_start_batch_result = server_request_call_event.call.start_server_batch(
                [
                    cygrpc.SendInitialMetadataOperation(
        ...
        with server_call_condition:
            server_complete_rpc_start_batch_result = server_request_call_event.call.start_server_batch(
                [
