...
class ReadSomeButNotAllResponsesTest(unittest.TestCase):
    def testReadSomeButNotAllResponses(self):
        ...
        server.start()
        channel = cygrpc.Channel("localhost:{}".format(port).encode(), set(), None)
        ...
        with server_call_condition:
            server_send_initial_metadata_start_batch_result = server_rpc_event.call.start_server_batch(
                [cygrpc.SendInitialMetadataOperation(_EMPTY_METADATA, _EMPTY_FLAGS),],
                server_send_initial_metadata_tag,
            )
            server_send_first_message_start_batch_result = server_rpc_event.call.start_server_batch(
        ...
        with server_call_condition:
            server_send_second_message_start_batch_result = server_rpc_event.call.start_server_batch(
                [cygrpc.SendMessageOperation(b"\x07", _EMPTY_FLAGS),],
                server_send_second_message_tag,
            )
            server_complete_rpc_start_batch_result = server_rpc_event.call.start_server_batch(
                [
                    cygrpc.ReceiveCloseOnServerOperation(_EMPTY_FLAGS),
                    cygrpc.SendStatusFromServerOperation(
                        (), cygrpc.StatusCode.ok, b"test details", _EMPTY_FLAGS
                    ),
                ],
                server_complete_rpc_tag,
            )
        ...
        self.assertEqual(
            cygrpc.CompletionType.operation_complete, server_rpc_event.completion_type
        )
