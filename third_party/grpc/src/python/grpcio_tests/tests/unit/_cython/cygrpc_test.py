...
class TypeSmokeTest(unittest.TestCase):
    ...
    def test_metadata_plugin_call_credentials_up_down(self):
        cygrpc.MetadataPluginCallCredentials(_metadata_plugin, b"test plugin name!")
...
class ServerClientMixin(object):
    ...
    def test_echo(self):
        ...
        self.assertEqual(
            cygrpc.CompletionType.operation_complete, request_event.completion_type
        )
        ...
        self.assertLess(
            abs(DEADLINE - request_event.call_details.deadline), DEADLINE_TOLERANCE
        )
        ...
        for client_result in client_event.batch_operations:
            ...
            if client_result.type() == cygrpc.OperationType.receive_initial_metadata:
                self.assertTrue(
                    test_common.metadata_transmitted(
                        server_initial_metadata, client_result.initial_metadata()
                    )
                )
                ...
                self.assertTrue(
                    test_common.metadata_transmitted(
                        server_trailing_metadata, client_result.trailing_metadata()
                    )
                )
    ...
    def test_6522(self):
        ...
        # Epilogue
        client_call.operate(
            [cygrpc.SendCloseFromClientOperation(_EMPTY_FLAGS),], "Client epilogue"
        )
        # One for ReceiveStatusOnClient, one for SendCloseFromClient.
        client_events_future = test_utilities.SimpleFuture(
            lambda: {client_call.next_event(), client_call.next_event(),}
        )
