...
class AuthContextTest(unittest.TestCase):
    ...
    def testSecureNoCert(self):
        ...
        channel = grpc.secure_channel(
            "localhost:{}".format(port), channel_creds, options=_PROPERTY_OPTIONS
        )
        response = channel.unary_unary(_UNARY_UNARY)(_REQUEST)
        server.stop(None)
        ...
        self.assertDictEqual(
            {"transport_security_type": [b"ssl"], "ssl_session_reused": [b"false"],},
            auth_data[_AUTH_CTX],
        )
    ...
    def testSecureClientCert(self):
        ...
        self.assertSequenceEqual([b"*.test.google.com"], auth_ctx["x509_common_name"])
