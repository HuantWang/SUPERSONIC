...
from SuperSonic.proto.grpc.testing import messages_pb2
...
def _expect_status_code(call, expected_code):
    if call.code() != expected_code:
        raise ValueError("expected code %s, got %s" % (expected_code, call.code()))
...
def _rst_after_header(stub):
    ...
    _validate_status_code_and_details(
        resp_future, grpc.StatusCode.INTERNAL, "Received RST_STREAM with error code 0"
    )
...
def _args():
    ...
    parser.add_argument(
        "--server_port", help="the port to which to connect", type=int, default="8080"
    )
    parser.add_argument(
        "--test_case", help="the test case to execute", type=str, default="goaway"
    )
