...
def _massage_proto_content(
    ...
    package_substitution = (
        b"package grpc_protoc_plugin.invocation_testing." + test_name_bytes + b";"
    )
    ...
    message_import_replaced = split_namespace_substituted.replace(
        _MESSAGES_IMPORT, b'import "' + messages_proto_relative_file_name_bytes + b'";'
    )
...
def _protoc(
    ...
    if grpc_python_out is not None:
        args.append(
            "--grpc_python_out={}:{}".format(grpc_python_out_flag, grpc_python_out)
        )
...
class _ProtoBeforeGrpcProtocStyle(object):
    ...
    def protoc(self, proto_path, python_out, absolute_proto_file_names):
        ...
        return (
            pb2_protoc_exit_code,
            pb2_grpc_protoc_exit_code,
        )
...
class _Test(six.with_metaclass(abc.ABCMeta, unittest.TestCase)):
    ...
    def _protoc(self):
        ...
        self._services_pb2_grpc = generated_modules[self.EXPECTED_SERVICES_PB2_GRPC]

    def _services_modules(self):
        if self.PROTOC_STYLE.grpc_in_pb2_expected():
            return (
                self._services_pb2,
                self._services_pb2_grpc,
            )
        else:
...
def _create_test_case_class(split_proto, protoc_style):
    ...
    if split_proto:
        ...
        attributes["SERVICES_PROTO_RELATIVE_DIRECTORY_NAMES"] = ("split_services",)
        ...
        attributes["EXPECTED_SERVICES_PB2_GRPC"] = "split_services.services_pb2_grpc"
    else:
        attributes["MESSAGES_PROTO_RELATIVE_DIRECTORY_NAMES"] = ()
