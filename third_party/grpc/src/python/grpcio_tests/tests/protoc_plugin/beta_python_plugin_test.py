...
def _massage_proto_content(raw_proto_content):
    imports_substituted = raw_proto_content.replace(
        b'import "tests/protoc_plugin/protos/', b'import "beta_grpc_plugin_test/'
    )
...
class PythonPluginTest(unittest.TestCase):
  ...
  """
    ...
    def setUp(self):
        ...
        for proto_file_path_components in _PROTO_FILES_PATH_COMPONENTS:
            raw_proto_content = pkgutil.get_data(
                "tests.protoc_plugin.protos", path.join(*proto_file_path_components[1:])
            )
    ...
    def testImportAttributes(self):
        ...
        self.assertIsNotNone(getattr(self._service_pb2, SERVICER_IDENTIFIER, None))
        ...
        self.assertIsNotNone(getattr(self._service_pb2, STUB_FACTORY_IDENTIFIER, None))

    def testUpDown(self):
        self._protoc()

        with _CreateService(self._payload_pb2, self._responses_pb2, self._service_pb2):
    ...
    def testStreamingOutputCall(self):
        ...
        with _CreateService(
            ...
            responses = stub.StreamingOutputCall(request, test_constants.LONG_TIMEOUT)
    ...
    def testStreamingInputCallFutureExpired(self):
        ...
        with _CreateService(
            ...
            with methods.pause():
                ...
                with self.assertRaises(face.ExpirationError):
                    response_future.result()
                self.assertIsInstance(response_future.exception(), face.ExpirationError)
