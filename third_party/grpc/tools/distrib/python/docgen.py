...
parser.add_argument(
    "--gh-repo-owner",
    type=str,
    help=("Owner of the GitHub repository to be pushed; " "defaults to --gh-user."),
)
...
if args.submit:
    ...
    github_repository_owner = args.gh_repo_owner if args.gh_repo_owner else args.gh_user
    ...
    subprocess.check_call(
        ["git", "checkout", "upstream/gh-pages", "-b", doc_branch], cwd=repo_dir
    )
    ...
    try:
        ...
        subprocess.check_call(
            ["git", "commit", "-m", "Auto-update Python documentation"], cwd=repo_dir
        )
        subprocess.check_call(
            ["git", "push", "--set-upstream", "origin", doc_branch], cwd=repo_dir
        )
