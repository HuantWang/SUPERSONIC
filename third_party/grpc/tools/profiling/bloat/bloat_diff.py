...
argp.add_argument(
    "-d", "--diff_base", type=str, help="Commit or branch to compare the current one to"
)
...
subprocess.check_call("make -j%d" % args.jobs, shell=True, cwd="third_party/bloaty")
...
for lib in LIBS:
    ...
    if old_version:
        ...
        text += subprocess.check_output("%s %s" % (cmd, new_version[0]), shell=True)
    text += "\n\n"
