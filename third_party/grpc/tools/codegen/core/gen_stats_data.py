...
def make_type(name, fields):
    return (
        collections.namedtuple(name, " ".join(list(set(REQUIRED_FIELDS + fields)))),
        [],
    )
...
def gen_bucket_code(histogram):
    ...
    if done_trivial:
        ...
        shift_data = find_ideal_shift(
            code_bounds[first_nontrivial:], 256 * histogram.buckets
        )
    ...
    if first_nontrivial is None:
        ...
        if shift_data is not None:
            ...
            code += "}\n"
        code += (
            "GRPC_STATS_INC_HISTOGRAM(GRPC_STATS_HISTOGRAM_%s, "
            % histogram.name.upper()
        )
        code += (
...
with open("src/core/lib/debug/stats_data.h", "w") as H:
    ...
    for i, tbl in enumerate(static_tables):
        print >> H, "extern const %s grpc_stats_table_%d[%d];" % (
            tbl[0],
            i,
            len(tbl[1]),
        )
...
with open("src/core/lib/debug/stats_data.cc", "w") as C:
    ...
    print >> C, "const int grpc_stats_histo_buckets[%d] = {%s};" % (
        ...
        ",".join("%s" % x for x in histo_buckets),
    )
    print >> C, "const int grpc_stats_histo_start[%d] = {%s};" % (
        len(inst_map["Histogram"]),
        ",".join("%s" % x for x in histo_start),
    )
    print >> C, "const int *const grpc_stats_histo_bucket_boundaries[%d] = {%s};" % (
        len(inst_map["Histogram"]),
        ",".join("grpc_stats_table_%d" % x for x in histo_bucket_boundaries),
    )
...
def AddCoreFields(js):
    for counter in inst_map["Counter"]:
        js["fields"].append(
            {"name": "core_%s" % counter.name, "type": "INTEGER", "mode": "NULLABLE"}
        )
    for histogram in inst_map["Histogram"]:
        js["fields"].append(
            {"name": "core_%s" % histogram.name, "type": "STRING", "mode": "NULLABLE"}
        )
        js["fields"].append(
            {
