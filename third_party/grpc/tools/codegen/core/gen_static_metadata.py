...
for mask in range(1, 1 << len(STREAM_COMPRESSION_ALGORITHMS)):
    val = ",".join(
        STREAM_COMPRESSION_ALGORITHMS[alg]
...
def slice_def(i):
    return ("{&grpc_static_metadata_refcounts[%d]," " {{g_bytes+%d, %d}}}") % (
        i,
        id2strofs[i],
        len(all_strs[i]),
    )
...
print >> H, (
    "  ((slice).refcount != NULL && (slice).refcount->vtable == "
...
for i, elem in enumerate(all_elems):
    print >> D, "%s" % (
        esc_dict(
            [0, len(elem[0])]
            + [ord(c) for c in elem[0]]
            + [len(elem[1])]
            + [ord(c) for c in elem[1]]
        )
    )
...
for i, elem in enumerate(all_elems):
    print >> H, '/* "%s": "%s" */' % elem
    print >> H, (
        "#define %s (GRPC_MAKE_MDELEM(&grpc_static_mdelem_table[%d], "
