...
def _scrape_for_known_errors(html):
    ...
    for known_error in _KNOWN_ERRORS:
        ...
        if this_error_count > 0:
            error_list.append({"description": known_error, "count": this_error_count})
...
def _process_matrix(build, url_base):
    ...
    for matrix in build.get_matrix_runs():
        matrix_str = re.match(".*\\xc2\\xbb ((?:[^,]+,?)+) #.*", matrix.name).groups()[
            0
        ]
...
for build_name in _BUILDS.keys() if "all" in args.builds else args.builds:
    ...
    starting_build_number = max(
        last_processed_build_number + 1, last_complete_build_number - 9
    )
    for build_number in xrange(starting_build_number, last_complete_build_number + 1):
        ...
        if not big_query_utils.insert_rows(
            bq, _PROJECT_ID, _DATASET_ID, build_name, rows
        ):
