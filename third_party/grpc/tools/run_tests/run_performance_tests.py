...
def create_scenario_jobspec(
    scenario_json, workers, remote_host=None, bq_result_table=None, server_cpu_load=0
):
...
def prepare_remote_hosts(hosts, prepare_local=False):
    ...
    num_failures, _ = jobset.run(prepare_jobs, newline_on_success=True, maxjobs=10)
    if num_failures == 0:
        jobset.message(
            "SUCCESS", "Prepare step completed successfully.", do_newline=True
        )
    else:
        jobset.message("FAILED", "Failed to prepare remote hosts.", do_newline=True)
        sys.exit(1)


def build_on_remote_hosts(
    hosts, languages=scenario_config.LANGUAGES.keys(), build_local=False
):
...
def perf_report_processor_job(
    ...
    if worker_host != "localhost":
        ...
        cmd = (
            "USER_AT_HOST=%s OUTPUT_FILENAME=%s OUTPUT_DIR=%s PERF_BASE_NAME=%stools/run_tests/performance/process_remote_perf_flamegraphs.sh"
            % (user_at_host, output_filename, flame_graph_reports, perf_base_name)
        )
    else:
        cmd = (
            "OUTPUT_FILENAME=%s OUTPUT_DIR=%s PERF_BASE_NAME=%stools/run_tests/performance/process_local_perf_flamegraphs.sh"
...
def main():
    ...
    if not args.dry_run:
        build_on_remote_hosts(
            remote_hosts, languages=[str(l) for l in languages], build_local=build_local
        )
        ...
        if args.dry_run:
            ...
            try:
                ...
                if scenario.workers:
                    jobs.append(
                        create_quit_jobspec(
                            scenario.workers, remote_host=args.remote_driver_host
                        )
                    )
                ...
                merged_resultset = dict(
                    itertools.chain(
                        six.iteritems(merged_resultset), six.iteritems(resultset)
                    )
                )
            finally:
            ...
            if (
                ...
                perf_report_failures += run_collect_perf_profile_jobs(
                    workers_and_base_names, scenario.name, args.flame_graph_reports
                )
