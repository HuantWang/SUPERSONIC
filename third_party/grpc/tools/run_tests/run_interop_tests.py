...
_SKIP_CLIENT_COMPRESSION = ["client_compressed_unary", "client_compressed_streaming"]

_SKIP_SERVER_COMPRESSION = ["server_compressed_unary", "server_compressed_streaming"]
...
class CSharpCoreCLRLanguage:
    def __init__(self):
        ...
        self.server_cwd = (
            "src/csharp/Grpc.IntegrationTesting.Server/bin/Debug/netcoreapp1.0"
        )
        self.safename = str(self)
...
def cloud_to_prod_jobspec(
    ...
    if auth:
        auth_cmdargs, auth_env = auth_options(
            language, test_case, service_account_key_file
        )
...
def build_interop_image_jobspec(language, tag=None):
    ...
    env = {"INTEROP_IMAGE": tag, "BASE_NAME": "grpc_interop_%s" % language.safename}
    if not args.travis:
        env["TTY_FLAG"] = "-t"
...
argp.add_argument(
    ...
    type=str,
    help="Override the default service account key file to use for auth interop tests.",
    default=None,
)
...
argp.add_argument(
    ...
    const=True,
    help=(
        "Put reports into subdirectories to improve "
...
try:
    ...
    num_failures, resultset = jobset.run(
        jobs, newline_on_success=True, maxjobs=args.jobs, skip_jobs=args.manual_run
    )
