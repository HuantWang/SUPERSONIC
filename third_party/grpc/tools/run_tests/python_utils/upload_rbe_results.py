...
def _upload_results_to_bq(rows):
  ...
  """
    ...
    for attempt in range(max_retries):
        if big_query_utils.insert_rows(bq, _PROJECT_ID, _DATASET_ID, _TABLE_ID, rows):
            break
        else:
...
def _get_resultstore_data(api_key, invocation_id):
  ...
  """
    ...
    while True:
        req = urllib2.Request(
            url="https://resultstore.googleapis.com/v2/invocations/%s/targets/-/configuredTargets/-/actions?key=%s&pageToken=%s"
...
if __name__ == "__main__":
    ...
    for action in resultstore_actions:
        ...
        if "fileProcessingErrors" in action:
            ...
            test_cases = action["testAction"]["testSuite"]["tests"][0]["testSuite"][
                "tests"
            ]
            ...
            bq_rows.append(
                {
                    ...
                    "json": {
                        ...
                        "build_url": "https://sponge.corp.google.com/invocation?id=%s"
