...
def start_port_server():
    ...
    if not running:
        ...
        while True:
            if waits > 10:
                logging.warning("killing port server due to excessive start up waits")
                port_server.kill()
            if port_server.poll() is not None:
                ...
                time.sleep(1)
                try:
                    urllib.urlopen("http://localhost:%d/get" % _PORT_SERVER_PORT).read()
                    logging.info("last ditch attempt to contact port server succeeded")
                    break
                except:
                    logging.exception("final attempt to contact port server failed")
