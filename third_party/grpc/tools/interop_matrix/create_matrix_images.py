...
def maybe_apply_patches_on_git_tag(stack_base, lang, release):
    ...
    for repo_relative_path in files_to_patch:
        subprocess.check_output(
            ["git", "add", repo_relative_path], cwd=stack_base, stderr=subprocess.STDOUT
        )
    subprocess.check_output(
        [
            "git",
...
def checkout_grpc_stack(lang, release):
    ...
    if not os.path.exists(stack_base):
        subprocess.check_call(
            ["git", "clone", "--recursive", repo], cwd=os.path.dirname(stack_base)
        )

    # git checkout.
    jobset.message(
        "START", "git checkout %s from %s" % (release, stack_base), do_newline=True
    )
    ...
    jobset.message(
        "SUCCESS", "git checkout", "%s: %s" % (str(output), commit_log), do_newline=True
    )
