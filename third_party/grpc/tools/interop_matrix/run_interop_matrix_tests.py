...
def find_all_images_for_lang(lang):
  ...
  """
    ...
    for runtime in client_matrix.LANG_RUNTIME_MATRIX[lang]:
        ...
        jobset.message(
            "START", "Found images for %s: %s" % (image_path, tags), do_newline=True
        )
        ...
        images[runtime] = [
            (tag, "%s:%s" % (image_path, tag)) for tag in tags if tag in releases
        ]
    return images
        ...
        with open(testcases) as f:
            ...
            for line in f.readlines():
                if line.startswith("docker run"):
                    ...
                    m = re.search(
                        "--server_host_override=(.*).sandbox.googleapis.com", line
                    )
