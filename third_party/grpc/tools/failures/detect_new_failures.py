...
def gh(url, data=None):
    ...
    if 200 <= response.getcode() < 300:
        ...
        raise ValueError(
            "Error ({}) accessing {}".format(response.getcode(), response.geturl())
        )
...
def print_table(table, format):
    ...
    for test_name, results_row in table.items():
        ...
        if format == "human":
            print(
                "\t- Test: {}, Polling: {}, Timestamp: {}, url: {}".format(
...
def process_date_args(args):
    ...
    return {
        "calibration": {"begin": calibration_begin, "end": calibration_end},
