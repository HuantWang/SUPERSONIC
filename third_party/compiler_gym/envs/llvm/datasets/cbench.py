...
def _compile_and_run_bitcode_file(
    ...
    if sanitizer:
        ...
        try:
            ...
            return BenchmarkExecutionResult(
                ...
                error=ValidationError(type="Compilation timeout", data=error_data,),
            )
        if clang.returncode:
            error_data["output"] = output
            return BenchmarkExecutionResult(
                walltime_seconds=timeout_seconds,
                error=ValidationError(type="Compilation failed", data=error_data,),
            )
    ...
    try:
        ...
        return BenchmarkExecutionResult(
            walltime_seconds=timeout_seconds,
            error=ValidationError(type="Execution timeout", data=error_data,),
        )
    finally:
        if sanitizer:
            binary.unlink()
    ...
    if process.returncode:
        ...
        return BenchmarkExecutionResult(
            ...
            error=ValidationError(type=error_type, data=error_data,),
        )
...
validator(
    benchmark="benchmark://cbench-v1/bitcount", cmd="$BIN 1125000",
)

validator(
    benchmark="benchmark://cbench-v1/bitcount", cmd="$BIN 512",
)
