...
flags.DEFINE_string(
    "heading_underline_char", "-", "The character to repeat to underline headings.",
)
FLAGS = flags.FLAGS
...
def summarize_datasets(datasets: Iterable[Dataset]) -> str:
    ...
    for dataset in sorted(datasets, key=lambda d: d.name):
        ...
        rows.append((dataset.name, dataset.size, description, dataset.validatable,))
    rows.append(("Total", sum(r[1] for r in rows), "", ""))
    return (
        tabulate(
            [
                (n, humanize.intcomma(f) if f >= 0 else "∞", l, v,)
                for n, f, l, v in rows
            ],
            headers=(
                "Dataset",
    ...
    for action_space in env.action_spaces:
        ...
        if isinstance(action_space, Commandline):
            table = tabulate(
                [
                    (f"`{n}`", d)
                    for n, d in zip(action_space.names, action_space.descriptions,)
                ],
                headers=("Action", "Description"),
            )
        else:
            table = tabulate(
                [(a,) for a in sorted(action_space.names)], headers=("Action",),
            )
        print(table)
