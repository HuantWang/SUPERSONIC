...
def init(
    ...
    ray.register_custom_serializer(
        RequestMetadata, RequestMetadata.ray_serialize, RequestMetadata.ray_deserialize
    )
    ...
    master_actor = ServeMaster.options(
        name=master_actor_name, max_restarts=-1, max_task_retries=-1,
    ).remote(name, http_node_id, http_host, http_port, metric_exporter)

    block_until_http_ready(
        "http://{}:{}/-/routes".format(http_host, http_port), timeout=HTTP_PROXY_TIMEOUT
    )


@_ensure_connected
def create_endpoint(endpoint_name, *, backend=None, route=None, methods=["GET"]):
    ...
    """
    if backend is None:
        raise TypeError("backend must be specified when creating " "an endpoint.")
    elif not isinstance(backend, str):
        raise TypeError("backend must be a string, got {}.".format(type(backend)))
    ...
    if not isinstance(methods, list):
        raise TypeError(
            "methods must be a list of strings, but got type {}".format(type(methods))
        )
    ...
    ray.get(
        master_actor.create_endpoint.remote(
...
def update_backend_config(backend_tag, config_options):
    ...
    ray.get(master_actor.update_backend_config.remote(backend_tag, config_options))
...
@_ensure_connected
def create_backend(
    backend_tag, func_or_class, *actor_init_args, ray_actor_options=None, config=None
):
    ...
    ray.get(
        master_actor.create_backend.remote(backend_tag, backend_config, replica_config)
    )
...
def set_traffic(endpoint_name, traffic_policy_dictionary):
    ...
    """
    ray.get(master_actor.set_traffic.remote(endpoint_name, traffic_policy_dictionary))
...
def get_handle(
    ...
    """
    if not missing_ok:
        assert endpoint_name in ray.get(master_actor.get_all_endpoints.remote())
