...
class TestSensorsBattery(unittest.TestCase):
    ...
    def test_emulate_no_base_files(self):
        ...
        with mock_open_exception(
            "/sys/class/power_supply/BAT0/energy_now", IOError(errno.ENOENT, "")
        ):
            with mock_open_exception(
                "/sys/class/power_supply/BAT0/charge_now", IOError(errno.ENOENT, "")
            ):
...
class TestProcess(unittest.TestCase):
    ...
    def test_memory_full_info(self):
        ...
        self.assertAlmostEqual(
            mem.uss, sum([x.private_dirty + x.private_clean for x in maps]), delta=4096
        )
    ...
    def test_rlimit_zombie(self):
        ...
        with mock.patch(
            "psutil._pslinux.cext.linux_prlimit", side_effect=OSError(errno.ENOSYS, "")
        ) as m:
            p = psutil.Process()
            p.name()
