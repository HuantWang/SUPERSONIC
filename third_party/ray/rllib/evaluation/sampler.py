...
def _env_runner(
    ...
    def get_batch_builder():
        if batch_builder_pool:
            ...
            return MultiAgentSampleBatchBuilder(policies, clip_rewards, callbacks)

    def new_episode():
        episode = MultiAgentEpisode(
            policies, policy_mapping_fn, get_batch_builder, extra_batch_callback
        )
...
def _process_observations(
    ...
    large_batch_threshold = (
        max(1000, rollout_fragment_length * 10)
        if rollout_fragment_length != float("inf")
        else 5000
    )
    ...
    for env_id, agent_obs in unfiltered_obs.items():
        ...
        if episode.batch_builder.total() > large_batch_threshold and log_once(
            "large_batch_warning"
        ):
            logger.warning(
                "More than {} observations for {} env steps ".format(
                    episode.batch_builder.total(), episode.batch_builder.count
                )
                + "are buffered in "
        ...
        for agent_id, raw_obs in agent_obs.items():
            ...
            prep_obs = _get_or_raise(preprocessors, policy_id).transform(raw_obs)
            ...
            if last_observation is not None and infos[env_id].get(agent_id, {}).get(
        ...
        if episode.batch_builder.has_pending_agent_data():
            ...
            if (
                all_agents_done and not pack_multiple_episodes_in_batch
            ) or episode.batch_builder.count >= rollout_fragment_length:
        ...
        if all_agents_done:
            ...
            if resetted_obs is None:
                ...
                for agent_id, raw_obs in resetted_obs.items():
                    ...
                    prep_obs = _get_or_raise(preprocessors, policy_id).transform(
                        raw_obs
                    )
                    filtered_obs = _get_or_raise(obs_filters, policy_id)(prep_obs)
                    episode._set_last_observation(agent_id, filtered_obs)
                    to_eval[policy_id].append(
                        PolicyEvalData(
                            ...
                            episode.rnn_state_for(agent_id),
                            np.zeros_like(
                                flatten_to_single_ndarray(policy.action_space.sample())
                            ),
                            0.0,
                        )
                    )
...
def _do_policy_eval(*, to_eval, policies, active_episodes, tf_sess=None):
    ...
    for policy_id, eval_data in to_eval.items():
        ...
        if builder and (
            policy.compute_actions.__code__ is TFPolicy.compute_actions.__code__
        ):
    ...
    *,
    ...
    for policy_id, eval_data in to_eval.items():
        ...
        for i, action in enumerate(actions):
            ...
            # Clip if necessary.
            if clip_actions:
                clipped_action = clip_action(action, policy.action_space_struct)
            else:
