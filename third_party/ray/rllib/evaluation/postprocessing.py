...
def compute_advantages(
    rollout, last_r, gamma=0.9, lambda_=1.0, use_gae=True, use_critic=True
):
    """
    ...
    if use_gae:
        ...
        discounted_returns = (
            discount(rewards_plus_v, gamma)[:-1].copy().astype(np.float32)
        )
    ...
    assert all(
        val.shape[0] == trajsize for val in traj.values()
