...
class ValueLoss:
    def __init__(self, state_values, cumulative_rewards):
        self.loss = 0.5 * tf.reduce_mean(tf.square(state_values - cumulative_rewards))


class ReweightedImitationLoss:
    def __init__(
        self, policy, state_values, cumulative_rewards, actions, action_dist, beta
    ):
        ...
        self.loss = -1.0 * tf.reduce_mean(tf.stop_gradient(exp_advs) * logprobs)
...
def postprocess_advantages(
    ...
    return compute_advantages(
        sample_batch, last_r, policy.config["gamma"], use_gae=False, use_critic=False
    )
...
class MARWILLoss:
    def __init__(
        ...
        self.p_loss = self._build_policy_loss(
            policy, state_values, advantages, actions, action_dist, beta
        )
    ...
    def _build_policy_loss(
        self, policy, state_values, cum_rwds, actions, action_dist, beta
    ):
        return ReweightedImitationLoss(
            policy, state_values, cum_rwds, actions, action_dist, beta
        )
