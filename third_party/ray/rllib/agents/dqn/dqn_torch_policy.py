...
class QLoss:
    def __init__(
        ...
        if num_atoms > 1:
            raise ValueError(
                "Torch version of DQN does not support " "distributional Q yet!"
            )
        ...
        self.loss = torch.mean(importance_weights.float() * huber_loss(self.td_error))
        self.stats = {
...
def build_q_stats(policy, batch):
    return dict({"cur_lr": policy.cur_lr,}, **policy.q_loss.stats)
...
def compute_q_values(policy, model, obs, explore, is_training=False):
    ...
    model_out, state = model(
        {SampleBatch.CUR_OBS: obs, "is_training": is_training,}, [], None
    )
