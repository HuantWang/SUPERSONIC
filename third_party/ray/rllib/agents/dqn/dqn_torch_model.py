...
class DQNTorchModel(TorchModelV2, nn.Module):
        ...
        *,
        ...
        if self.dueling:
            ...
            for i, n in enumerate(q_hiddens):
                advantage_module.add_module("dueling_A_{}".format(i), nn.Linear(ins, n))
                value_module.add_module("dueling_V_{}".format(i), nn.Linear(ins, n))
                # Add activations if necessary.
                if dueling_activation == "relu":
                    advantage_module.add_module("dueling_A_act_{}".format(i), nn.ReLU())
                    value_module.add_module("dueling_V_act_{}".format(i), nn.ReLU())
                elif dueling_activation == "tanh":
                    advantage_module.add_module("dueling_A_act_{}".format(i), nn.Tanh())
                    value_module.add_module("dueling_V_act_{}".format(i), nn.Tanh())
    ...
    def _noisy_layer(self, action_in, out_size, sigma0, non_linear=True):
        ...
        sigma_b = torch.Tensor(
            data=np.full(shape=[out_size], fill_value=sigma0 / np.sqrt(float(in_size))),
            requires_grad=True,
        )
        w = torch.Tensor(
            data=np.full(
        ...
        action_activation = (
            torch.matmul(action_in, w + sigma_w * epsilon_w) + b + sigma_b * epsilon_b
        )
