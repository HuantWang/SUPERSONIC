...
class AsyncSamplesOptimizer(PolicyOptimizer):
        ...
        if num_gpus > 1 or num_data_loader_buffers > 1:
            ...
            if num_data_loader_buffers < minibatch_buffer_size:
                raise ValueError(
                    ...
                    "{} vs {}".format(num_data_loader_buffers, minibatch_buffer_size)
                )
    ...
    def stats(self):
        ...
        stats["timing_breakdown"] = {
            ...
            "learner_load_wait_time_ms": timer_to_ms(self.learner.load_wait_timer),
    ...
    def _step(self):
        ...
        for train_batch in self.aggregator.iter_train_batches():
            ...
            if self.learner.weights_updated and self.aggregator.should_broadcast():
