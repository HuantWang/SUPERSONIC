        ...
        def learn_on_batch(self, samples):
            ...
            if self._traced_learn_on_batch is None:
                self._traced_learn_on_batch = tf.function(
                    super(TracedEagerPolicy, self).learn_on_batch, autograph=False
                )
            ...
            **kwargs
            ...
            if self._traced_compute_actions is None:
                self._traced_compute_actions = tf.function(
                    super(TracedEagerPolicy, self).compute_actions, autograph=False
                )
        ...
        def compute_gradients(self, samples):
            ...
            if self._traced_compute_gradients is None:
                self._traced_compute_gradients = tf.function(
                    super(TracedEagerPolicy, self).compute_gradients, autograph=False
                )
        ...
        def apply_gradients(self, grads):
            ...
            if self._traced_apply_gradients is None:
                self._traced_apply_gradients = tf.function(
                    super(TracedEagerPolicy, self).apply_gradients, autograph=False
                )
    ...
    class eager_policy_cls(base):
        def __init__(self, observation_space, action_space, config):
            ...
            input_dict = {
                ...
                SampleBatch.PREV_REWARDS: tf.convert_to_tensor([0.0]),
            }
            ...
            **kwargs
            ...
            if obs_include_prev_action_reward:
                if prev_action_batch is not None:
                    input_dict[SampleBatch.PREV_ACTIONS] = tf.convert_to_tensor(
                        prev_action_batch
                    )
                if prev_reward_batch is not None:
                    input_dict[SampleBatch.PREV_REWARDS] = tf.convert_to_tensor(
                        prev_reward_batch
                    )
            ...
            with tf.variable_creator_scope(_disallow_var_creation):
                if action_sampler_fn:
                    ...
                    if action_distribution_fn:
                        dist_inputs, dist_class, state_out = action_distribution_fn(
                            self,
                            self.model,
                            input_dict[SampleBatch.CUR_OBS],
                            explore=explore,
        ...
        def _compute_gradients(self, samples):
            ...
            with tf.GradientTape(persistent=gradients_fn is not None) as tape:
                ...
                model_out, _ = self.model(samples, self._state_in, self._seq_lens)
            ...
            if gradients_fn:
                ...
                grads_and_vars = gradients_fn(self, OptimizerWrapper(tape), loss)
            else:
                grads_and_vars = list(zip(tape.gradient(loss, variables), variables))
        ...
        def _initialize_loss_with_dummy_batch(self):
            ...
            def tile_to(tensor, n):
                return tf.tile(tensor, [n] + [1 for _ in tensor.shape.as_list()[1:]])

            if get_batch_divisibility_req:
                dummy_batch = tf.nest.map_structure(
                    lambda c: tile_to(c, get_batch_divisibility_req(self)), dummy_batch
                )
