...
class TestRolloutWorker(unittest.TestCase):
    ...
    def test_batch_ids(self):
        ...
        self.assertEqual(len(set(SampleBatch.concat(batch1, batch2)["unroll_id"])), 2)
    ...
    def test_global_vars_update(self):
        ...
        for fw in framework_iterator(frameworks=()):
            ...
            for i in range(10):
                result = agent.train()
                print(
                    "num_steps_sampled={}".format(result["info"]["num_steps_sampled"])
                )
                print(
                    "num_steps_trained={}".format(result["info"]["num_steps_trained"])
                )
                print(
                    "num_steps_sampled={}".format(result["info"]["num_steps_sampled"])
                )
                ...
                if i == 0:
                    self.assertGreater(
                        result["info"]["learner"]["default_policy"]["cur_lr"], 0.01
                    )
                if result["info"]["learner"]["default_policy"]["cur_lr"] < 0.07:
                    break
            self.assertLess(result["info"]["learner"]["default_policy"]["cur_lr"], 0.07)
    ...
    def test_no_step_on_init(self):
        ...
        for fw in framework_iterator(frameworks=()):
            pg = PGTrainer(env="fail", config={"num_workers": 1, "framework": fw,})
