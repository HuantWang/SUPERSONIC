...
class TrXLNet(RecurrentNetwork):
    ...
    def __init__(
        ...
        for _ in range(self.num_transformer_units):
            ...
            E_out = SkipConnection(PositionwiseFeedforward(attn_dim, ff_hidden_dim))(
                MHA_out
            )
...
class GTrXLNet(RecurrentNetwork):
    ...
    def forward_rnn(self, inputs, state, seq_lens):
        ...
        if self.memory_tau > self.max_seq_len:
            memory_outs = [
                tf.concat(
                    [memory[i][:, -(self.memory_tau - self.max_seq_len) :], m], axis=1
                )
                for i, m in enumerate(memory_outs)
            ]
        else:
...
def relative_position_embedding(seq_length, out_dim):
    ...
    pos_offsets = tf.range(seq_length - 1.0, -1.0, -1.0)
