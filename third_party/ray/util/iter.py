...
class ParallelIterator(Generic[T]):
    ...
    def batch(self, n: int) -> "ParallelIterator[List[T]]":
        ...
        """
        return self._with_transform(
            lambda local_it: local_it.batch(n), ".batch({})".format(n)
        )
    ...
    def union(self, other: "ParallelIterator[T]") -> "ParallelIterator[T]":
        ...
        if not isinstance(other, ParallelIterator):
            raise TypeError(
                "other must be of type ParallelIterator, got {}".format(type(other))
            )
        actor_sets = []
        ...
        *others: "LocalIterator[T]",
        ...
        for it in others:
            if not isinstance(it, LocalIterator):
                raise ValueError(
                    "other must be of type LocalIterator, got {}".format(type(it))
                )
        ...
        return LocalIterator(
            ...
            shared_metrics,
            [],
            name="LocalUnion[{}, {}]".format(self, ", ".join(map(str, others))),
        )
