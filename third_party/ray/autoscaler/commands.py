...
def teardown_cluster(
    config_file, yes, workers_only, override_cluster_name, keep_min_workers
):
    ...
    try:
        ...
        def remaining_nodes():
            ...
            if keep_min_workers:
                ...
                logger.info(
                    "teardown_cluster: " "Keeping {} nodes...".format(min_workers)
                )
...
def attach_cluster(
    config_file, start, use_screen, use_tmux, override_cluster_name, new, port_forward
):
...
def _exec(updater, cmd, screen, tmux, port_forward=None, with_output=False):
    if cmd:
        if screen:
            cmd = ["screen", "-L", "-dm", "bash", "-c", quote(cmd + "; exec bash")]
            cmd = " ".join(cmd)
        elif tmux:
            # TODO: Consider providing named session functionality
            cmd = ["tmux", "new", "-d", "bash", "-c", quote(cmd + "; exec bash")]
            cmd = " ".join(cmd)
    return updater.cmd_runner.run(
        cmd, exit_on_fail=True, port_forward=port_forward, with_output=with_output
    )
...
def _get_worker_nodes(config, override_cluster_name):
    ...
    provider = get_node_provider(config["provider"], config["cluster_name"])
    try:
        return provider.non_terminated_nodes({TAG_RAY_NODE_TYPE: NODE_TYPE_WORKER})
    finally:
        provider.cleanup()


def _get_head_node(config, config_file, override_cluster_name, create_if_needed=False):
    provider = get_node_provider(config["provider"], config["cluster_name"])
    try:
        head_node_tags = {
            TAG_RAY_NODE_TYPE: NODE_TYPE_HEAD,
