...
def dockerize_if_needed(config):
    ...
    config["head_start_ray_commands"] = docker_autoscaler_setup(
        cname
    ) + with_docker_exec(config["head_start_ray_commands"], container_name=cname)
...
def with_docker_exec(cmds, container_name, env_vars=None):
    ...
    return [
        "docker exec {} {} /bin/sh -c {} ".format(env_str, container_name, quote(cmd))
        for cmd in cmds
    ]
...
def docker_autoscaler_setup(cname):
    ...
    for path in ["~/ray_bootstrap_config.yaml", "~/ray_bootstrap_key.pem"]:
        ...
        cmds.append(
            "docker cp {path} {cname}:{dpath}".format(
                path=path, dpath=base_path, cname=cname
            )
        )
        cmds.extend(
            with_docker_exec(
                ["cp {} {}".format("/" + base_path, path)], container_name=cname
            )
        )
    return cmds
