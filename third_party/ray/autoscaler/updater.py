...
class SSHCommandRunner:
    ...
    def run(
        self, cmd, timeout=120, exit_on_fail=False, port_forward=None, with_output=False
    ):
        ...
        if cmd:
            logger.info(
                self.log_prefix + "Running {} on {}...".format(cmd, self.ssh_ip)
            )
...
class NodeUpdater:
    ...
    def do_update(self):
        ...
        if node_tags.get(TAG_RAY_RUNTIME_CONFIG) == self.runtime_hash:
            logger.info(
                self.log_prefix
                + "{} already up-to-date, skip to ray start".format(self.node_id)
            )
        else:
            self.provider.set_node_tags(
