...
def wait_for_compute_zone_operation(compute, project_name, operation, zone):
    ...
    for _ in range(MAX_POLLS):
        result = (
            ...
            .get(project=project_name, operation=operation["name"], zone=zone)
            .execute()
        )
        if "error" in result:
...
class GCPNodeProvider(NodeProvider):
    ...
    def create_node(self, base_config, tags, count):
        with self.lock:
            ...
            machine_type = "zones/{zone}/machineTypes/{machine_type}" "".format(
                zone=availability_zone, machine_type=base_config["machineType"]
            )
    ...
    def terminate_node(self, node_id):
        with self.lock:
            ...
            operation = (
                self.compute.instances()
                .delete(project=project_id, zone=availability_zone, instance=node_id,)
                .execute()
            )
