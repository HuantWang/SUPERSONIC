...
DEFAULT_SERVICE_ACCOUNT_ROLES = ("roles/storage.objectAdmin", "roles/compute.admin")
...
def wait_for_compute_global_operation(project_name, operation):
    ...
    for _ in range(MAX_POLLS):
        result = (
            ...
            .get(project=project_name, operation=operation["name"],)
            .execute()
        )
        if "error" in result:
            raise Exception(result["error"])
...
def _configure_subnet(config):
    ...
    if (
        "networkInterfaces" in config["head_node"]
...
def _create_service_account(account_id, account_config, config):
    ...
    service_account = (
        ...
        .create(
            ...
            body={"accountId": account_id, "serviceAccount": account_config,},
        )
        .execute()
    )
