...
class NodeStats(threading.Thread):
    ...
    def _calculate_log_counts(self):
        return {
            ip: {pid: len(logs_for_pid) for pid, logs_for_pid in logs_for_ip.items()}
    ...
    def _purge_outdated_stats(self):
        ...
        self._node_stats = {
            k: v for k, v in self._node_stats.items() if current(v["now"], now)
        }
    ...
    def get_node_stats(self):
        with self._node_stats_lock:
            ...
            node_stats = sorted(
                (v for v in self._node_stats.values()), key=itemgetter("boot_time")
            )
            return {
                "clients": node_stats,
    ...
    def get_actor_tree(self, workers_info_by_node, infeasible_tasks, ready_tasks):
        now = time.time()
        ...
        with self._node_stats_lock:
            for addr, actor_id in self._addr_to_actor_id.items():
                ...
                flattened_tree[actor_id].update(self._addr_to_extra_info_dict[addr])
            ...
            for infeasible_task in infeasible_tasks:
                _update_flatten_tree(
                    infeasible_task, "actorCreationTaskSpec", "infeasibleActor"
                )

            for ready_task in ready_tasks:
                _update_flatten_tree(
                    ready_task, "actorCreationTaskSpec", "pendingActor"
                )
    ...
    def run(self):
        ...
        with self._node_stats_lock:
            for actor_data in current_actor_table.values():
                addr = (
                    actor_data["Address"]["IPAddress"],
                    str(actor_data["Address"]["Port"]),
                )
                owner_addr = (
                    actor_data["OwnerAddress"]["IPAddress"],
            ...
            try:
                with self._node_stats_lock:
                    ...
                    if channel == log_channel:
                        ...
                        logger.warning(
                            "Unexpected channel data received, "
                            "channel: {}, data: {}".format(
                                channel, json.loads(ray.utils.decode(data))
                            )
                        )
