...
def check_no_existing_redis_clients(node_ip_address, redis_client):
    ...
    for key in client_keys:
        ...
        if ray.utils.decode(info[b"node_ip_address"]) == node_ip_address:
            raise Exception(
                "This Redis instance is already connected to "
...
@click.option("--redis-address", required=False, type=str, help="same as --address")
...
@click.option(
    ...
    type=int,
    help=(
        "the number of additional Redis shards to use in "
...
@click.option(
    ...
    type=int,
    help=(
        "If provided, attempt to configure Redis with this "
...
@click.option(
    "--include-java", is_flag=True, default=None, help="Enable Java worker support."
)
@click.option(
    "--java-worker-options",
    required=False,
...
def start(
    ...
    if redis_address is not None or address is not None:
        (
            ...
            redis_address_ip,
            redis_address_port,
        ) = services.validate_redis_address(address, redis_address)
    ...
    if head:
        ...
        logger.info(
            "Using IP address {} for this node.".format(ray_params.node_ip_address)
        )
        ...
        if num_redis_shards is not None:
            raise Exception(
                "If --head is not passed in, --num-redis-shards "
                "must not be provided."
            )
        if redis_max_clients is not None:
            raise Exception(
                "If --head is not passed in, --redis-max-clients "
                "must not be provided."
            )
        if include_webui:
            raise Exception(
        ...
        if include_java is not None:
            raise ValueError("--include-java should only be set for the head " "node.")
...
def attach(cluster_config_file, start, screen, tmux, cluster_name, new, port_forward):
    """Create or attach to a SSH session to a Ray cluster."""
    port_forward = [(port, port) for port in list(port_forward)]
    attach_cluster(
        cluster_config_file, start, screen, tmux, cluster_name, new, port_forward
    )
...
def timeline(address):
    ...
    filename = os.path.join(
        ray.utils.get_user_temp_dir(), "ray-timeline-{}.json".format(time)
    )
...
def stat(address):
    ...
    for raylet in ray.nodes():
        ...
        reply = stub.GetNodeStats(
            node_manager_pb2.GetNodeStatsRequest(include_memory_info=False), timeout=2.0
        )
        print(reply)
...
try:
    ...
    logger.debug("Integrating ray serve command line tool failed with {}".format(e))
