...
class JupyterNotebookReporter(TuneReporterBase):
    ...
    def __init__(
        ...
        super(JupyterNotebookReporter, self).__init__(
            metric_columns, max_progress_rows, max_error_rows, max_report_frequency
        )
...
def trial_progress_str(trials, metric_columns, fmt="psql", max_rows=None):
    ...
    if num_trials > max_rows:
        ...
        overflow = False
    messages.append(
        "Number of trials: {} ({})".format(num_trials, ", ".join(num_trials_strs))
    )
    ...
    if isinstance(metric_columns, Mapping):
        ...
        keys = metric_columns
    keys = [k for k in keys if any(t.last_result.get(k) is not None for t in trials)]
    # Build trial rows.
...
def _fair_filter_trials(trials_by_state, max_trials):
    ...
    sorted_trials_by_state = {
        state: sorted(trials_by_state[state], reverse=False, key=lambda t: t.trial_id)
        for state in sorted(trials_by_state)
    }
