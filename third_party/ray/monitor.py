...
class Monitor:
    ...
    def xray_heartbeat_batch_handler(self, unused_channel, data):
        ...
        message = ray.gcs_utils.HeartbeatBatchTableData.FromString(heartbeat_data)
        for heartbeat_message in message.batch:
            resource_load = dict(
                zip(
                    heartbeat_message.resource_load_label,
                    heartbeat_message.resource_load_capacity,
                )
            )
            total_resources = dict(
                zip(
                    heartbeat_message.resources_total_label,
                    heartbeat_message.resources_total_capacity,
                )
            )
            available_resources = dict(
                zip(
                    heartbeat_message.resources_available_label,
            ...
            if ip:
                ...
                logger.warning(
                    "Monitor: " "could not find ip for client {}".format(client_id)
                )
    ...
    def xray_job_notification_handler(self, unused_channel, data):
        ...
        if message.is_dead:
            logger.info(
                "Monitor: "
                "XRay Driver {} has been removed.".format(binary_to_hex(job_id))
            )
    ...
    def update_raylet_map(self, _append_port=False):
        ...
        for raylet_info in all_raylet_nodes:
            ...
            ip_address = (
                raylet_info.get("AuxAddress") or raylet_info["NodeManagerAddress"]
            ).split(":")[0]
            if _append_port:
    ...
    def _run(self):
        ...
        while True:
            ...
            # messages.
            time.sleep(ray._config.raylet_heartbeat_timeout_milliseconds() * 1e-3)
...
if __name__ == "__main__":
    ...
    monitor = Monitor(
        args.redis_address, autoscaling_config, redis_password=args.redis_password
    )
