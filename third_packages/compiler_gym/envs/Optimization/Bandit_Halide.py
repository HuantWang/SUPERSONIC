...
import sys
import SuperSonic.bin.utils.engine.tasks_engine as start_engine
import shutil
import os
import json
import sqlite3
class BanditHalideEnv(gym.Env):
    ...
    def get_observation(self,action):
        ...
        if len(self.actions) < self.obsv_size:
            self.observation = np.hstack((np.array(self.actions), np.zeros(self.obsv_size - len(self.actions))))[:self.obsv_size]
