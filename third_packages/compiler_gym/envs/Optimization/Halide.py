...
class HalideEnv(gym.Env):
    ...
    def __init__(self,algorithm_id,input_image,max_stage_directive,log_path,state_function,action_function,reward_function):  #get
        ...
        f = open(os.path.join(log_path, "result.csv"), "w+", encoding="utf-8", newline="")
        ...
        self.obsv_size = 128
        self.action_space, self.observation_space = action_functions().init_actions(response.schedule_map_range,
        ...
        self.map = np.empty(self.observation_space.shape, dtype=self.observation_space.dtype)
    ...
    def get_obs(self, reward, done, error):
        ...
        if not done and not error and self.min_exec_time_sec < self.best_exec_time_sec:
            ...
            epinfo = {"t": round(time.time() - self.tstart, 6), "best_exec": info[ek],
                      "best_schedule": info[sk]}
            print(epinfo)
            if csv_writer:
                print("enter write")
                csv_writer.writerow([round(time.time() - self.tstart, 6), info[ek], info[sk]])
                f.flush()
        ...
        sql = "INSERT INTO HALIDE (TIME, RESULT,ACTIONS,REWARD,LOG) \
                                      VALUES (?, ?, ?, ?, ?)"
        c.execute(sql,(round(time.time() - self.tstart),code, str(self.actions), reward, self.min_exec_time_sec ))
        ...
        self.state = observation_function().get_observation(self.input,self.obsv_size,self.state_function)

        if not done and not error and self.map_count >= (self.max_stage * self.max_stage_directive):
            done = True
